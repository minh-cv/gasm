...
void gasm::ToBeta::transpile() {
    for (const auto& stmt: parse_result.stmts) {
        std::visit(*this, stmt);
    }
}

void gasm::ToBeta::operator()(const Verbatim& v) {
    stream << v.data;
}

std::string_view gasm::ToBeta::trim_reg_expr(const RegExpr& expr) {
...
std::string_view gasm::ToBeta::trim_expr(const Expr& expr, int trim_precedence) {
...
void gasm::ToBeta::operator()(const Move& m) {
...
void gasm::ToBeta::operator()(const Store& st) {
...
void gasm::ToBeta::operator()(const Load& st) {
    stream << "LD(";
    if (st.mem.reg == std::nullopt) {
        assert(st.mem.expr != std::nullopt);
        stream << st.mem.expr->data;
...
void gasm::ToBeta::operator()(const Inst& inst) {
...
void gasm::ToBeta::operator()(const Goto& g) {
...
void gasm::ToBeta::operator()(const Op& op) {
...
void gasm::ToGamma::transpile() {
    for (const auto& stmt: parse_result.stmts) {
        std::visit(*this, stmt);
    }
}

void gasm::ToGamma::operator()(const Verbatim& v) {
    stream << v.data;
}

void gasm::ToGamma::operator()(const Move& m) {
    stream << m.rc.data << " = ";
    if (std::holds_alternative<RegExpr>(m.src)) {
...
void gasm::ToGamma::print_mem(const MemExpr& expr) {
    stream << "[";
...
void gasm::ToGamma::operator()(const Store& st) {
    print_mem(st.mem);
    stream << " = " << st.reg.data;
}

void gasm::ToGamma::operator()(const Load& st) {
    stream << st.reg.data << " = " ;
    print_mem(st.mem);
}

RegExpr gasm::ToGamma::transform_reg_expr(const std::variant<Expr, RegExpr>& var, std::string& buffer) {
...
Expr gasm::ToGamma::transform_expr(const std::variant<Expr, RegExpr>& var, std::string& buffer, int transform_precedence) {
...
void gasm::ToGamma::operator()(const Inst& inst) {
    std::string buffer[3];
    ...
    case 0x37:
        operator()(Op{
            *get_type(inst.identifier.lexeme), 
            transform_reg_expr(inst.params[0], buffer[0]), // buffer[0] mutated, string_view 0 constructed 
            transform_reg_expr(inst.params[1], buffer[1]), // buffer[1] mutated, string_view 1 constructed
            transform_reg_expr(inst.params[2], buffer[2]) // buffer[2] mutated, string_view 2 constructed
        }); // all 3 string_view destructed
        break;
    case 0x35: {
...
void gasm::ToGamma::operator()(const Goto& g) {
    if (g.cond != std::nullopt) {
        stream << "if (";
...
void gasm::ToGamma::operator()(const Op& op) {
...
void gasm::ToGamma::operator()(const OpC& op) {
